...
class Model(nn.Module):
    def __init__(self, params, bert):
        ...
        if self.typ_embed:
            self.one_hot_embed = nn.Embedding(3, 96)
            self.proj_typ = nn.Linear(96, self.in_dim)

        # Graph Encoding Module
        self.global_encoder = AHT(self.in_dim, self.Conv, self.layer_norm, self.dropout)
        self.local_encoder = FAGCN_MOE(self.in_dim, self.eps)
    ...
    def forward(self, input_ids, input_masks, node_types,
                g_0, g_1, g_2, target_ids, add_ids, pertub):

        x_seq = self.embed(input_ids, input_masks)[1]
        x_seq = self.proj_seq(x_seq)

        if self.typ_embed:
            x_typ = self.one_hot_embed(node_types)
            x_typ = self.proj_typ(x_typ)
            x = F.relu(x_typ + x_seq)
        else:
            x = F.relu(x_seq)

        x = self.global_encoder(x, target_ids, add_ids)
        g_feat1, scores = self.local_encoder(x, g_0, g_1, g_2, target_ids, pertub)
        pred = self.mlp(g_feat1)
        pred = pred.squeeze(1)

        if pertub:
            g_feat2, _ = self.local_encoder(x, g_0, g_1, g_2, target_ids, pertub)
            return pred, g_feat1, g_feat2
        else:
            # return pred, scores, g_feat1
            return g_feat1
...
class FAGCN_MOE(nn.Module):
    def __init__(self, in_dim, eps):
        ...
        self.gate = nn.Linear(self.in_dim, 3)
    ...
    def forward(self, x, g_0, g_1, g_2, target_ids, pertub):
        ...
        experts = torch.cat([s_out, a_out, b_out], dim=1)
        gate_out = torch.matmul(gate, experts).squeeze(1)
        x = self.lin(x)
        x = self.dp(F.relu(x))
...
class AHT(nn.Module):
    def __init__(self, dim, Conv, layer_norm, dropout):
        ...
        self.pool = MAB(self.dim, self.n_heads, self.Conv, self.layer_norm)
...
class MAB(nn.Module):
    def __init__(self, dim, n_heads, Conv, layer_norm):
