...
from utils import load_data, eval_line, train_batch, test_batch
...
def parse_args():
    ...
    parser.add_argument('-max_node', type=int, default=50,
                        help="The max number of nodes in a code graph.")

    parser.add_argument("-max_token", type=int, default=20,
                        help="The length of the code corresponding to a node in the statement graph")

    parser.add_argument('-epochs', type=int, default=3,
                        help="The number of epochs for training.")

    parser.add_argument('-in_dim', type=int, default=512,
                        help='The dimension of embedding vector for a text encoder')
...
def train(train_data, bert, params):
    ...
    for epoch in range(1, params.epochs + 1):
        ...
        for batch in tqdm(batches):
            ...
            node_types = torch.tensor(graphs.node_types).to(params.device)
            ...
            predicts, g_feat1, g_feat2 = model.forward(input_ids, input_masks, node_types, g_0, g_1, g_2, target_ids, pertub=True)
...
def test(test_data, bert, params):
    ...
    model.eval()
    with torch.no_grad():
        all_commits, all_files, all_predicts, all_attns = [], [], [], []
        ...
        for batch in batches:
            ...
            add_labels = graphs.add_labels
            add_nums = graphs.add_nums

            predicts, attns = model.forward(input_ids, input_masks, node_types, g_0, g_1, g_2, target_ids, pertub=False)
            predicts = predicts.cpu().detach().numpy().tolist()

            for index, attn in zip(add_ids, attns):
            ...
            all_labels += labels
            all_add_ids += add_ids
            all_add_labels += add_labels
            all_add_nums += add_nums

        preds = [1 if pred >= 0.5 else 0 for pred in all_predicts]
        probs = all_predicts

        # file_result = []
        # for c, f, l, pred, prob in zip(all_commits, all_files, all_labels, preds, probs):
        #     file_result.append([c, f, l, pred, prob])
        #
        # cols = ['commit', 'file', 'label', 'prediction', 'probability']
        # pd.DataFrame(file_result, columns=cols).to_csv('data/ours_file_result_{}.txt'.format(params.out), sep='\t', index=False)
        eval_line(all_commits, all_files, all_labels, preds, probs, all_add_ids, all_add_labels, all_add_nums, all_attns, params.out)
