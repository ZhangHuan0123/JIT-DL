...
# ##### 仅评估缺陷文件 #####
# def eval_line(commits, files, labels, preds, probs, add_ids, add_tags, add_nums, attns, out):
#     line_result = []
#     for commit, file, label, pred, prob, ids, tags, nums, scores in zip(commits, files, labels, preds, probs, add_ids, add_tags, add_nums, attns):
#         if label == 1:
#             scores = get_scores(ids, scores, prob)
#             for num, score, tag in zip(nums, scores, tags):
#                 line_result.append([commit, file, num, score, label])
#     pd.DataFrame(line_result, columns=['commit', 'file', 'id', 'score', 'label']).to_csv('data/ours_line_result_{}.txt'.format(out), sep='\t', index=False)
...
def eval_line(commits, files, labels, preds, probs, add_ids, add_tags, add_nums, attns, out):
    ...
    for commit, file, label, pred, prob, ids, tags, nums, scores in zip(commits, files, labels, preds, probs, add_ids, add_tags, add_nums, attns):
        ...
        for num, score, tag in zip(nums, scores, tags):
            line_result.append([commit, file, num, score, label])
    pd.DataFrame(line_result, columns=['commit', 'file', 'id', 'score', 'label']).to_csv('data/{}.txt'.format(out), sep='\t', index=False)
